...
session id: 019e5fc8-ac25-76a0-b1cb-0840efba955a
--------
user
Reviewing Step 3 Phase 2 — shared defender pool by reference + ref-counted cleanup + last-kill-credit. Three besiegers attacking the same colony now share ONE defender pool (no doubling). Cleanup of any single besieger does NOT despawn shared entities if siblings still active. When defenders hit zero in tick(), the currently-processed raid wins; siblings lose with cooldown. Review for: (1) is the share-by-reference safe if primary's collection is mutated by background threads while secondaries iterate? (2) ref-counted cleanup correctness — what if cleanup runs from tick (iterator removes) vs from shutdown (no iterator)? (3) race in tick(): the iteration order of COLONY_RAID_INDEX is non-deterministic; is it OK that the order determines winner? (4) if launchRaid fails AFTER getRaidsForColony() returned the primary but BEFORE adding self to COLONY_RAID_INDEX, does cleanup correctly skip despawning shared state? STATUS APPROVE/REWORK, up to 5 bullets, up to 3 fixes. Max 300 words.
...
- Share-by-reference is structurally safe for iteration because the shared sets are `ConcurrentHashMap.newKeySet()`; iterators are weakly consistent and will not throw on concurrent mutation. But publication is risky: secondary raids are added to `ACTIVE_RAIDS` / `COLONY_RAID_INDEX` before their pool refs are reassigned, so another tick/cleanup can observe empty private sets.

- Ref-counted cleanup is not correct for race-loser cleanup during `tick()`. After the winner calls `cleanupRaid`, it removes itself from `COLONY_RAID_INDEX`; then the first loser sees siblings size maybe `2`, skips despawn, removes itself; the final loser sees size `1`, despawns the shared pool while still inside the same tick path. That is probably intended, but order-dependent and fragile.

- Shutdown is safer because `shutdown()` calls `cleanupRaid(raid, false)` for each active raid before clearing maps, so the last cleanup should despawn. However, because iteration is over `ACTIVE_RAIDS.values()` while cleanup mutates only the index, it should not trip iterator removal issues.

- Last-kill-credit is currently non-deterministic: `ACTIVE_RAIDS` / `COLONY_RAID_INDEX` are concurrent maps/sets with unspecified order. If product accepts “first processed this tick wins,” that is OK, but it is not “landed the killing blow” in a deterministic sense. The player-facing message overclaims.

- Launch failure after finding primary but before adding self does skip shared despawn because cleanup is not called, only `ACTIVE_RAIDS.remove` and `removeFromColonyIndex`. Good. But failure after assigning shared refs and adding index can still make cleanup decisions depend on partial publication.

**Fixes**

1. Assign secondary shared refs before publishing to `ACTIVE_RAIDS` / `COLONY_RAID_INDEX`, or guard launch/tick/cleanup with one server-thread lock.

2. Snapshot `COLONY_RAID_INDEX` winners/losers, remove all involved raids from index first, then perform shared cleanup exactly once.

3. Change messaging to “resolved first this tick” or implement deterministic credit by timestamp/damage tracker.
tokens used
20.667
**STATUS: REWORK**
