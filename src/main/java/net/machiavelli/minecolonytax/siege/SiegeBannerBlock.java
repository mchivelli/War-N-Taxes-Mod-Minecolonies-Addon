 ...
 * but explicitly breakable by defenders via melee (handled via the BlockBreak
 * event by {@code PlantTheBannerObjective}). Hardness 0.5 so a quick left-click
 * destroys it for defenders who get past attackers.
 *
 * NeoForge 1.21.1 port of the Forge {@code SiegeBannerBlock}. The only override
 * is {@link #setPlacedBy}, whose signature is unchanged in 1.21, so no migration
 * to {@code useItemOn}/{@code useWithoutItem} is required. {@code Properties.of()}
 * does not require {@code .setId(...)} on this NeoForge build (mirrors how the
 * bundled MineColonies blocks construct their {@code Properties}).
 */
