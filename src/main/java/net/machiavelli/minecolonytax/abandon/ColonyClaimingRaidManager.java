 ...
 * and attack the claiming player. If there are too few defenders, mercenaries
 * are spawned.
 */
public class ColonyClaimingRaidManager {
    ...
    // Track claiming grace periods per player (UUID -> timestamp of last successful
    // claim)
    private static final Map<UUID, Long> claimingGracePeriods = new ConcurrentHashMap<>();

    // Track protected colonies that cannot be claimed (colony ID -> admin who
    // protected it)
    ...
    public static boolean startClaimingRaid(IColony colony, ServerPlayer claimingPlayer) {
        ...
        if (activeClaimingRaids.containsKey(colony.getID())) {
            claimingPlayer
                    .sendSystemMessage(Component.literal("A claiming raid is already in progress for this colony!")
                            .withStyle(ChatFormatting.RED));
            return false;
        }
        ...
        boolean isFormerMember = ColonyAbandonmentManager.wasFormerOwnerOrOfficer(colony.getID(),
                claimingPlayer.getUUID());
        if (isFormerMember) {
            ...
            claimingPlayer.sendSystemMessage(Component.literal(
                    "You are reclaiming your former colony. Requirements bypassed but you must complete the claiming raid!")
                    .withStyle(ChatFormatting.YELLOW));
        }

        try {
            // Create raid data
            ClaimingRaidData raidData = new ClaimingRaidData(colony.getID(), claimingPlayer.getUUID(),
                    colony.getCenter());
            ...
            if (TaxConfig.shouldSpawnMercenariesIfLowDefenders() && citizenCount < 5) {
                ...
                if (mercenaryCount > 0) {
                    ...
                    LOGGER.info(
                            "Updated total defenders to {} (citizens: {}, mercenaries: {}) for claiming raid in colony {}",
            ...
            MutableComponent startMessage = Component.literal("COLONY CLAIMING RAID STARTED!")
                    ...
                    .append(Component
                            .literal("\n" + claimingPlayer.getName().getString()
                                    + " is attempting to claim the abandoned colony of ")
                            .withStyle(ChatFormatting.YELLOW))
    ...
    /**
     * Convert all citizens in the colony to hostile militia that attack the
     * claiming player.
     */
    private static int convertCitizensToMilitia(IColony colony, ServerPlayer claimingPlayer,
            ClaimingRaidData raidData) {
        int convertedCount = 0;

        // Initialize militia system for this colony if needed
        net.machiavelli.minecolonytax.militia.CitizenMilitiaManager.getInstance()
                .initializeColonyMilitia(colony.getID());
     ...
     */
    private static int spawnDefendingMercenaries(IColony colony, ServerPlayer claimingPlayer,
            ClaimingRaidData raidData) {
        try {
            ...
            for (int i = 0; i < mercenaryCount; i++) {
                EntityMercenary mercenary = (EntityMercenary) com.minecolonies.api.entity.ModEntities.MERCENARY
                        .create(world);
                if (mercenary == null)
                    continue;
    ...
    /**
     * Create a boss bar to track the claiming raid progress with timer and defender
     * count.
    ...
    private static void createRaidBossBar(ClaimingRaidData raidData, ServerPlayer claimingPlayer) {
        ...
        try {
            ...
            raidData.bossEvent = new ServerBossEvent(bossBarText, BossEvent.BossBarColor.GREEN,
            ...
            try {
                ...
                LOGGER.info(
                        "BOSS BAR CREATED: Player {} added to boss bar for colony {} ({} defenders, {} minute timer)",
                        ...
                        if (player.level() == world && !player.equals(claimingPlayer)) {
                            ...
                            if (distance <= 100 * 100) { // 100 block radius
                                try {
                                    ...
                                    playersAdded++;
                                    LOGGER.debug("Added nearby player {} to claiming raid boss bar",
                                            player.getName().getString());
                                } catch (Exception e) {
                                    LOGGER.warn("Failed to add nearby player {} to boss bar: {}",
            ...
            // Force an immediate update to ensure the boss bar shows correctly with current
            // time
            try {
                updateRaidBossBar(raidData);
                LOGGER.info("BOSS BAR INITIALIZED: Successfully created and updated boss bar for colony {}",
                        colony.getName());
            } catch (Exception e) {
    ...
    public static void updateClaimingRaids() {
        ...
        while (iterator.hasNext()) {
            ...
            // CRITICAL: Timer expiration is ALWAYS a failure - attackers must kill all
            // defenders to win
     ...
     * Shows time as a countdown timer and defender count for clear victory
     * conditions.
     */
    private static void updateRaidBossBar(ClaimingRaidData raidData) {
        ...
        // Progress bar represents TIME REMAINING (full = lots of time, empty = time
        // almost up)
     ...
     * VICTORY CONDITION: Attackers can ONLY win by killing ALL defenders - no other
     * victory conditions exist!
     */
    private static void checkRaidConditions(ClaimingRaidData raidData) {
        try {
            ...
            ServerPlayer claimingPlayer = getPlayerById(raidData.claimingPlayerId);
            if (claimingPlayer == null) {
                // Player might be offline or in a different dimension - don't end the raid
                // immediately
                // Just skip this update cycle and try again next time
                LOGGER.debug("Claiming player {} not found during raid condition check - skipping update",
                        raidData.claimingPlayerId);
                return;
            }
            ...
            // CRITICAL CHECK: Count all living defenders - attackers must kill EVERY SINGLE
            // ONE
            ...
            // VICTORY CONDITION: All defenders must be dead - no shortcuts, no idle
            // victories!
    ...
    public static void endClaimingRaid(ClaimingRaidData raidData, String reason) {
        try {
            ...
            if (claimingPlayer != null && !reason.equals("Player or colony not found")
                    && !reason.equals("Colony not found")) {
     ...
     * CRITICAL: This method ensures NO COLONY CORRUPTION by preserving all players
     * and only changing ranks.
     */
    public static void completeClaimingRaid(ClaimingRaidData raidData, boolean success) {
        try {
            ...
            LOGGER.info(
                    "COLONY CORRUPTION PREVENTION: Safely updating permissions for colony {} without removing any players",
                    colony.getName());
            ...
            if (wasAlreadyInColony) {
                ...
                permissions.addPlayer(claimingPlayer.getUUID(), claimingPlayer.getName().getString(),
            ...
            try {
                ...
                LOGGER.warn("Could not set claiming player as actual owner directly, trying alternative: {}",
                        e.getMessage());
                try {
            ...
            // STEP 2: Restore normal permissions for neutral players (they were restricted
            // during abandonment)
            ...
            LOGGER.info("Colony {} permissions safely updated - claimer is Officer, all players preserved",
                    colony.getName());
            ...
            if (isFormerMember) {
                ...
                successMessage = Component.literal("COLONY RECLAIMED!")
                        ...
                        .append(Component
                                .literal("\n" + claimingPlayer.getName().getString()
                                        + " has reclaimed their former colony ")
                                .withStyle(ChatFormatting.YELLOW))
                ...
                personalMessage = Component
                        .literal("Welcome back! You have successfully reclaimed your former colony " +
                                colony.getName()
                                + " and are now an Officer! All existing players remain in the colony. " +
                ...
                successMessage = Component.literal("COLONY CLAIMED SUCCESSFULLY!")
                        ...
                        .append(Component
                                .literal("\n" + claimingPlayer.getName().getString()
                                        + " has successfully claimed the abandoned colony of ")
                                .withStyle(ChatFormatting.YELLOW))
                        .append(Component.literal(colony.getName() + " and is now an Officer!")
                                .withStyle(ChatFormatting.GOLD))
                        .append(Component.literal("\nAll existing players remain with their current ranks.")
    ...
    /**
     * Force check victory conditions for a specific colony (can be called
     * externally).
     */
    ...
    /**
     * Check if a player meets the claiming requirements for a specific colony and
    ...
    public static ClaimingRequirementResult checkClaimingRequirements(ServerPlayer player, IColony targetColony) {
        ...
        if (lastClaimTime != null) {
            ...
            if (timeSinceLastClaim < gracePeriodMs) {
                ...
                String timeRemaining = remainingHours > 0 ? remainingHours + "h " + remainingMinutes + "m"
                        : remainingMinutes + "m";
        ...
        if (isFormerOwnerOrOfficer) {
            LOGGER.info(
                    "CLAIMING BYPASS: Player {} was former owner/officer of colony {} - bypassing requirements but must complete raid",
                    player.getName().getString(), targetColony != null ? targetColony.getName() : "unknown");
            return new ClaimingRequirementResult(true,
                    "Former owner/officer of this colony - requirements bypassed! You must still complete the claiming raid to reclaim control.");
        }
        ...
        net.machiavelli.minecolonytax.requirements.BuildingRequirementsManager.RequirementResult buildingCheck = net.machiavelli.minecolonytax.requirements.BuildingRequirementsManager
                .checkClaimingRequirements(playerColony);
    ...
    public static void setClaimingInteractionPermissions(IColony colony, boolean allowed) {
        ...
        LOGGER.info("Set claiming raid permissions for colony {} to: {} (includes attack permissions)",
                colony.getName(), allowed);
    }
                ...
                try {
                    ...
                    if (colony == null) {
                        ...
                        reason = "boss bar is null";
                            ...
                            - raidData.startTime > (TaxConfig.getClaimingRaidDurationMinutes() * 60000L + 300000L)) {
                        needsCleanup = true;
                        reason = "raid overtime (+" + ((System.currentTimeMillis() - raidData.startTime) / 60000L)
                                + " minutes)";
                    }
