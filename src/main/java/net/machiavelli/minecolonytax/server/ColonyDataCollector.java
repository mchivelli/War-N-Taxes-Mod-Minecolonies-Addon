...
public class ColonyDataCollector {
            ...
            for (VassalIncomeData data : vassalData) {
                ...
                try {
                    ...
                    if (vassalColony != null) {
                        ...
                        canClaim = (war == null) && !isBeingRaided
        ...
        try {
            ...
            try {
                ...
                if (isVassal || hasVassals) {
                    LOGGER.info(
                            "Colony {} (Player: {}) - isVassal: {}, vassalRate: {}, hasVassals: {}, vassalCount: {}",
            ...
            // Calculate approximate revenue per interval based on building count and tax
            // rates
            ...
            // Tax Policy
            net.machiavelli.minecolonytax.economy.TaxPolicyManager.TaxRate rate = net.machiavelli.minecolonytax.economy.TaxPolicyManager
                    .getEffectiveTaxRate(colonyId);
            String taxPolicy = rate.name();
            double taxPolicyMultiplier = 1.0
                    + net.machiavelli.minecolonytax.economy.TaxPolicyManager.getRevenueModifier(colonyId);

            // Happiness
            double happiness = TaxManager.calculateColonyAverageHappiness(colony);
            double happinessMultiplier = TaxConfig.calculateHappinessTaxMultiplier(happiness);
    ...
    private static int calculateApproximateRevenue(int buildingCount, int guardTowerCount) {
        ...
        // Real calculation uses actual colony happiness from
        // TaxManager.calculateColonyAverageHappiness()
        ...
        if (TaxConfig.isHappinessTaxModifierEnabled()) {
            ...
            // Real calculation:
            // TaxConfig.calculateHappinessTaxMultiplier(colonyAvgHappiness)
