...
public class ColonyUpgradeManager {
    ...
    public static boolean purchase(int colonyId, UpgradeType type, MinecraftServer server) {
        ...
        synchronized (lock) {
            ...
            // War-chest purchase (Neo economy): reject negative cost (int-overflow guard),
            // require sufficient balance, then deduct atomically inside this per-colony lock.
            if (cost < 0) return false;
            if (WarChestManager.getWarChestBalance(colonyId) < cost) return false;
            WarChestManager.deductFromWarChest(colonyId, cost);
    ...
    /**
     * Flat war-chest capacity bonus from the TREASURY_CAP investment. Returns {@code long}: the
     ...
     * (WarChestManager.getEffectiveMaxCapacity), which does its own [0, Integer.MAX_VALUE] clamp.
     */
