...
public class WntCommands {
        ...
        public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
                dispatcher.register(Commands.literal("wnt")
                                ...
                                // Tax Rate Commands
                                .then(Commands.literal("taxrate")
                                                .executes(WntCommands::showTaxRate) // Show current tax rate
                                                .then(Commands.argument("colony", StringArgumentType.string())
                                                                .suggests(PLAYER_COLONY_SUGGESTIONS)
                                                                .executes(ctx -> showTaxRateForColony(ctx,
                                                                                extractColonyName(StringArgumentType
                                                                                                .getString(ctx, "colony"))))
                                                                .then(Commands.literal("low")
                                                                                .executes(ctx -> setTaxRate(ctx,
                                                                                                extractColonyName(
                                                                                                                StringArgumentType
                                                                                                                                .getString(ctx, "colony")),
                                                                                                "low")))
                        ...
                        if (playerRank != null && playerRank.isColonyManager()) {
                                ...
                                if (claimedAmount > 0) {
                                        ...
                                        if (TaxConfig.isSDMShopConversionEnabled()) {
                                                ...
                                                net.sixik.sdmshoprework.SDMShopR.setMoney(player,
                                                                currentBalance + claimedAmount);
                                        } else {
        ...
        private static int debugTaxBreakdown(CommandContext<CommandSourceStack> context, String colonyName)
                ...
                try {
                        ...
                        // 2. Tax Policy
                        double policyModifier = TaxPolicyManager.getRevenueModifier(colonyId);
                        double policyMultiplier = 1.0 + policyModifier;
                        TaxPolicyManager.TaxRate policyRate = TaxPolicyManager.getEffectiveTaxRate(colonyId);
                        ...
                        if (happinessEnabled) {
                                ...
                                source.sendSuccess(() -> Component.literal("  Disabled (Config)")
                                                .withStyle(ChatFormatting.GRAY), false);
                        }

                        // Policy
                        source.sendSuccess(() -> Component.literal("\n📜 Tax Policy: " + policyRate.name())
                                        .withStyle(ChatFormatting.AQUA, ChatFormatting.BOLD), false);
                        source.sendSuccess(() -> Component.literal(String.format("  Multiplier: %.2fx (%+.0f%%)",
                                        policyMultiplier, policyModifier * 100))
                                        .withStyle(policyModifier > 0 ? ChatFormatting.GOLD
                                                        : (policyModifier < 0 ? ChatFormatting.GREEN
                                                                        : ChatFormatting.WHITE)),
                                        false);

                        // Penalties
                        if (warExhaustionMultiplier < 1.0 || raidPenaltyMultiplier < 1.0) {
                                source.sendSuccess(() -> Component.literal("\n⚔️ Penalties:")
                                                .withStyle(ChatFormatting.RED, ChatFormatting.BOLD), false);

                                if (warExhaustionMultiplier < 1.0) {
                                        source.sendSuccess(() -> Component
                                                        .literal(String.format("  War Exhaustion: %.2fx (-%.0f%%)",
                                                                        warExhaustionMultiplier,
                                                                        (1.0 - warExhaustionMultiplier) * 100))
                                                        .withStyle(ChatFormatting.RED), false);
                                }
                                if (raidPenaltyMultiplier < 1.0) {
                                        source.sendSuccess(() -> Component
                                                        .literal(String.format("  Raid Penalty: %.2fx (-%.0f%%)",
                                                                        raidPenaltyMultiplier,
                                                                        (1.0 - raidPenaltyMultiplier) * 100))
                                                        .withStyle(ChatFormatting.RED), false);
                                }
                        }
                        ...
                        for (com.minecolonies.api.colony.buildings.IBuilding building : colony.getBuildingManager()
                                ...
                                if (building.getBuildingLevel() > 0 && building.isBuilt()) {
                                        ...
                                        String displayName = building.getBuildingDisplayName(); // For display only
                                        ...
                                        // OLD logic: int generatedTax = (int) (rawTax * happinessMultiplier);

                                        // NEW logic: Apply all multipliers
                                        double taxWithHappiness = rawTax * happinessMultiplier;
                                        int generatedTax = (int) (taxWithHappiness * raidPenaltyMultiplier
                                                        * warExhaustionMultiplier * policyMultiplier);
                                        ...
                                        if (buildingCount <= 15) { // Show first 15 buildings
                                                final String bDisplay = displayName + " [" + buildingType + "]";
                        ...
                        source.sendSuccess(() -> Component.literal("  Total Buildings: " + finalBuildingCount)
                                        .withStyle(ChatFormatting.WHITE), false);
                        source.sendSuccess(
                                        () -> Component.literal("  Base Tax (raw): " + finalTotalBaseTax)
                                                        .withStyle(ChatFormatting.WHITE),
                                        false);
                        source.sendSuccess(() -> Component
                                        .literal("  Generated Tax (final): " + finalTotalGeneratedTax)
                                        .withStyle(ChatFormatting.GREEN), false);

                        source.sendSuccess(() -> Component.literal("     (Base * Happiness * Policy * Penalties)")
        ...
        /**
         * Show tax rate for all colonies the player manages
         */
        private static int showTaxRate(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
                ServerPlayer player = context.getSource().getPlayerOrException();
                CommandSourceStack source = context.getSource();
                IColonyManager colonyManager = IMinecoloniesAPI.getInstance().getColonyManager();

                List<IColony> playerColonies = colonyManager.getAllColonies().stream()
                                .filter(colony -> colony.getPermissions().getRank(player.getUUID()).isColonyManager())
                                .collect(Collectors.toList());

                if (playerColonies.isEmpty()) {
                        source.sendFailure(Component.literal("You don't manage any colonies!")
                                        .withStyle(ChatFormatting.RED));
                        return 0;
                }

                source.sendSuccess(() -> Component.literal("§6§l=== Tax Rate Status ==="), false);

                for (IColony colony : playerColonies) {
                        int colonyId = colony.getID();
                        net.machiavelli.minecolonytax.economy.TaxPolicyManager.TaxRate effectiveRate = net.machiavelli.minecolonytax.economy.TaxPolicyManager
                                        .getEffectiveTaxRate(colonyId);
                        net.machiavelli.minecolonytax.economy.TaxPolicyManager.TaxRate storedRate = net.machiavelli.minecolonytax.economy.TaxPolicyManager
                                        .getStoredTaxRate(colonyId);

                        boolean isWarMode = effectiveRate == net.machiavelli.minecolonytax.economy.TaxPolicyManager.TaxRate.WAR;
                        String rateDisplay = effectiveRate.name() + (isWarMode ? " (auto)" : "");
                        String description = net.machiavelli.minecolonytax.economy.TaxPolicyManager
                                        .getTaxRateDescription(effectiveRate);

                        ChatFormatting color = switch (effectiveRate) {
                        ...
                        source.sendSuccess(() -> Component.literal("  " + colony.getName() + ": ")
                                        .withStyle(ChatFormatting.WHITE)
                                        .append(Component.literal(rateDisplay).withStyle(color)), false);
                        source.sendSuccess(() -> Component.literal("    " + description).withStyle(ChatFormatting.GRAY),
                                        false);

                        if (isWarMode && storedRate != net.machiavelli.minecolonytax.economy.TaxPolicyManager.TaxRate.WAR) {
                                source.sendSuccess(() -> Component
                                                .literal("    (Set to " + storedRate.name() + " when not at war)")
                                                .withStyle(ChatFormatting.DARK_GRAY), false);
                        }
                }

                source.sendSuccess(() -> Component.literal("§7Use: /wnt taxrate <colony> <low|normal|high>"), false);
                return 1;
        }

        /**
         * Show tax rate for a specific colony
         */
        private static int showTaxRateForColony(CommandContext<CommandSourceStack> context, String colonyName)
                        throws CommandSyntaxException {
                ServerPlayer player = context.getSource().getPlayerOrException();
                CommandSourceStack source = context.getSource();
                IColonyManager colonyManager = IMinecoloniesAPI.getInstance().getColonyManager();
                ...
                // Check permission
                if (!colony.getPermissions().getRank(player.getUUID()).isColonyManager() && !source.hasPermission(2)) {
                        source.sendFailure(
                                        Component.literal("You don't have permission to view this colony's tax rate!")
                                                        .withStyle(ChatFormatting.RED));
                        return 0;
                }
                ...
                source.sendSuccess(() -> Component.literal("§6Tax Rate for " + finalColony.getName() + ":"), false);
                source.sendSuccess(
                                () -> Component.literal("  Current Rate: " + effectiveRate.name()
                                                + (isWarMode ? " (auto-war)" : ""))
                                                .withStyle(isWarMode ? ChatFormatting.RED : ChatFormatting.GREEN),
                                false);
                source.sendSuccess(() -> Component.literal("  Effect: " + description).withStyle(ChatFormatting.GRAY),
                                false);

                if (isWarMode && storedRate != net.machiavelli.minecolonytax.economy.TaxPolicyManager.TaxRate.WAR) {
                        source.sendSuccess(() -> Component.literal("  After war ends: " + storedRate.name())
                                        .withStyle(ChatFormatting.YELLOW), false);
                }
        ...
        /**
         * Set tax rate for a colony
         */
        private static int setTaxRate(CommandContext<CommandSourceStack> context, String colonyName, String rateString)
                        throws CommandSyntaxException {
                ServerPlayer player = context.getSource().getPlayerOrException();
                CommandSourceStack source = context.getSource();
                IColonyManager colonyManager = IMinecoloniesAPI.getInstance().getColonyManager();
                ...
                if (!colony.getPermissions().getRank(player.getUUID()).isColonyManager() && !source.hasPermission(2)) {
                        source.sendFailure(
                                        Component.literal("You don't have permission to change this colony's tax rate!")
                                                        .withStyle(ChatFormatting.RED));
                        return 0;
                }

                // Parse rate
                net.machiavelli.minecolonytax.economy.TaxPolicyManager.TaxRate rate;
                try {
                        rate = net.machiavelli.minecolonytax.economy.TaxPolicyManager.TaxRate
                                        .valueOf(rateString.toUpperCase());
                } catch (IllegalArgumentException e) {
                        source.sendFailure(
                                        Component.literal("Invalid tax rate! Use: low, normal, or high")
                                                        .withStyle(ChatFormatting.RED));
                        return 0;
                }

                if (rate == net.machiavelli.minecolonytax.economy.TaxPolicyManager.TaxRate.WAR) {
                        source.sendFailure(
                                        Component.literal(
                                                        "WAR rate cannot be set manually - it activates automatically during war!")
                                                        .withStyle(ChatFormatting.RED));
                        return 0;
                }
                ...
                // Set the rate
                int colonyId = colony.getID();
                boolean success = net.machiavelli.minecolonytax.economy.TaxPolicyManager.setTaxRate(colonyId, rate);

                if (success) {
                        String description = net.machiavelli.minecolonytax.economy.TaxPolicyManager
                                        .getTaxRateDescription(rate);
                        final IColony finalColony = colony;
                        source.sendSuccess(
                                        () -> Component.literal("§aTax rate for " + finalColony.getName() + " set to: "
                                                        + rate.name()),
                                        false);
                        source.sendSuccess(() -> Component.literal("§7" + description), false);

                        // Check if colony is at war
                        if (net.machiavelli.minecolonytax.economy.TaxPolicyManager.isInWarMode(colonyId)) {
                                source.sendSuccess(
                                                () -> Component.literal(
                                                                "§c(Currently in WAR mode - new rate will apply when war ends)")
                                                                .withStyle(ChatFormatting.ITALIC),
                                                false);
                        }
                } else {
                        source.sendFailure(Component.literal("Failed to set tax rate. Tax policies may be disabled.")
                                        .withStyle(ChatFormatting.RED));
                }
