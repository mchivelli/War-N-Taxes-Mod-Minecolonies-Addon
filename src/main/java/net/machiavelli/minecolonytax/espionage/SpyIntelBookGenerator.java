...
/**
 * Generates a Book and Quill (writable) intel report so the player can add their own notes.
 * Pages are plain text — no color formatting codes. The player may sign it once annotated.
 */
public class SpyIntelBookGenerator {
    ...
    public static ItemStack createIntelReport(SpyMission mission, String targetColonyName) {
        ...
        // Blank notes page — player writes their own annotations here
        pages.add("[ Notes ]\n" + SEP + "\n\n\n\n\n\n\n\n\n\n");

        ItemStack book = new ItemStack(Items.WRITABLE_BOOK);

        // 1.21 DataComponents API: WritableBookContent uses Filterable<String>
        List<Filterable<String>> pageComponents = new ArrayList<>();
        for (String page : pages) {
            pageComponents.add(Filterable.passThrough(page));
        }
        book.set(DataComponents.WRITABLE_BOOK_CONTENT, new WritableBookContent(pageComponents));

        // Set display name — writable books default to "Book and Quill"
        book.set(DataComponents.CUSTOM_NAME,
                Component.literal("Intelligence: " + targetColonyName)
    ...
    // ── Page 2: Tier 1 — Field Observations ─────────────────────────────────
