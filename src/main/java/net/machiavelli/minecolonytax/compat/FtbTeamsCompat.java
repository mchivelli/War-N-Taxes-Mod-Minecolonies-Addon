 ...
 *
 * <p>FTB Teams is an OPTIONAL dependency at runtime. Direct
 * {@code dev.ftb.mods.ftbteams.*} imports anywhere outside this package will
 * fail class loading (via {@link NoClassDefFoundError}) BEFORE any runtime
 * guard can run when FTB Teams is absent on the modpack. To prevent that, all
 * FTB Teams API calls go through this class.
 *
 ...
 * used for JourneyMap.
 *
 * <p><b>NeoForge port note:</b> only the ModList package changed
 * ({@code net.neoforged.fml.ModList}). FTB Teams IS a compile dependency in
 * this repo (build.gradle line 111), so the lazy-impl split is technically not
 * required for compilation, but it is preserved to keep the runtime
 * soft-dependency contract identical to the Forge original (a server that
 * strips ftbteams from the jar set still degrades gracefully).
 */
