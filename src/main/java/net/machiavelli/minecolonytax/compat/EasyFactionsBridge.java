 ...
 * the ModList package ({@code net.neoforged.fml.ModList}). Easy Factions is NOT
 * a compile dependency in either repo — every touch is reflective string-based
 * lookup, so no Easy Factions jar is needed to build. {@code TaxConfig.isNormalLogging()}
 * exists in the NeoForge config and is used unchanged.
 */
