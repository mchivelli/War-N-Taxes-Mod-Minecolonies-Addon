...
public class InvestmentsPage extends BookPage {
    ...
    public void onActivated() {
        ...
        if (colony != null) {
            NetworkHandler.sendToServer(new RequestInvestmentDataPacket(colony.getColonyId()));
        }
    }
    ...
    // ── Left page: scrollable type list ──────────────────────────────────────
    ...
    public void renderRightPage(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        g.drawString(font, "Treasury:", rightX + 3, y + 3, INK_FAINT, false);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (selectedType != null) {
            ...
            if (canAfford && isInBounds(mouseX, mouseY, rightX, buyButtonY, rightW, BUY_BTN_H)) {
                ColonyTaxData c = selectedColonySupplier.get();
                if (c != null) {
                    NetworkHandler.sendToServer(new BuyInvestmentPacket(c.getColonyId(), selectedType.name()));
                    // Refresh investment data so levels and treasury balance update live
                    NetworkHandler.sendToServer(new RequestInvestmentDataPacket(c.getColonyId()));
                }
                return true;
            }
        }
