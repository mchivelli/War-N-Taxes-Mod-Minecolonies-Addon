...
import net.machiavelli.minecolonytax.network.packets.DeploySpyPayload;
import net.machiavelli.minecolonytax.network.packets.DismissSpyMissionPayload;
import net.machiavelli.minecolonytax.network.packets.RecallSpyPayload;
import net.machiavelli.minecolonytax.network.packets.RequestSpyDataPayload;
...
public class EspionagePage extends BookPage {
    ...
    // Per-type colors for button text
    private static final int[] MISSION_COLORS = {BLUE, DANGER, GOLD_DARK, GREEN};
    // Accurate descriptions based on actual game mechanics
    ...
    // Minecraft widget for target colony input
    ...
    private void onTargetTextChanged(String text) {
        ...
        for (int i = 0; i < colonies.size(); i++) {
            if (colonies.get(i).getColonyName().toLowerCase().startsWith(lower)) {
                targetColonyIndex = i;
                return;
            }
        }
        ...
        for (int i = 0; i < colonies.size(); i++) {
            if (colonies.get(i).getColonyName().toLowerCase().contains(lower)) {
                targetColonyIndex = i;
                return;
            }
        }
    ...
    public void onActivated() {
        ...
        NetworkHandler.sendToServer(new RequestSpyDataPayload());
    }
    ...
    public void renderLeftPage(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        for (int i = 0; i < visible; i++) {
            ...
            // For active missions: show 3-segment tier progress on the right
    ...
    public void renderRightPage(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        // Manage EditBox visibility based on current view
        ...
        // If a mission is selected, show detail + action buttons
        if (!showDeployView) {
        ...
        // Show matched colony confirmation
        List<ColonyTaxData> colonies = coloniesSupplier.get();
        if (targetColonyIndex >= 0 && targetColonyIndex < colonies.size()) {
            String match = truncate(font, "\u2192 " + colonies.get(targetColonyIndex).getColonyName(), w - 4);
        ...
        // Mission type selector with per-type colors
        ...
        // Mission type description (bottom space)
    ...
    private int renderMissionDetail(GuiGraphics g, SpyMissionData mission, int x, int y, int w,
        ...
        // Header: colony name + status badge
        ...
        if ("ACTIVE".equals(status)) {
            ...
            // 3-segment progress bar (each segment = one intel tier)
            int barW = w - 4;
            int segSpacing = 2;
            int nSegs = 3;
        ...
        // COMPLETED / ESCAPED / RECALLED -- show intel
        if (intel == null) {
        ...
        // Tier 2: Guards, morale, treasury
        if (intel.isMidAvailable()) {
            drawSeparator(g, x, y, w); y += 4;

            // Guards with color context
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (isInRightPage(mouseX, mouseY)) {
            // If viewing a mission detail: back button, recall, delete
            if (selectedMissionIndex >= 0 && selectedMissionIndex < missions.size()) {
                ...
                if (canRecall && isInBounds(mouseX, mouseY, rightX, actionBtnY, rightW / 2, 11)) {
                    NetworkHandler.sendToServer(new RecallSpyPayload(mission.getMissionId()));
                    return true;
                }

                // Delete button (right half, only for finished missions)
                boolean canDismiss = isFinished(status);
                if (canDismiss && isInBounds(mouseX, mouseY, rightX + rightW / 2 + 2, actionBtnY, rightW / 2 - 2, 11)) {
                    NetworkHandler.sendToServer(new DismissSpyMissionPayload(mission.getMissionId()));
                    selectedMissionIndex = -1;
                    if (targetInput != null) targetInput.setVisible(true);
                    return true;
                }
            ...
            if (isInBounds(mouseX, mouseY, rightX, deployY, w, 12)) {
                if (targetColonyIndex >= 0 && targetColonyIndex < colonies.size()) {
                    ColonyTaxData target = colonies.get(targetColonyIndex);
                    NetworkHandler.sendToServer(new DeploySpyPayload(target.getColonyId(),
