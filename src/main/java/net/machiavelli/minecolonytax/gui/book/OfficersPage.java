...
public class OfficersPage extends BookPage {
    ...
    public void onActivated() {
        ...
        if (colony != null) {
            NetworkHandler.sendToServer(new RequestOfficerDataPacket(colony.getColonyId()));
        }
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (isInRightPage(mouseX, mouseY) && colony != null && colony.isOwner()
            ...
            if (isInBounds(mouseX, mouseY, x + w - 30, defaultToggleY, 28, 11)) {
                ...
                NetworkHandler.sendToServer(new UpdateTaxPermissionPacket(colony.getColonyId(), newPerm));
                // Refresh officer data so the toggle state updates live
                NetworkHandler.sendToServer(new RequestOfficerDataPacket(colony.getColonyId()));
                return true;
            }
