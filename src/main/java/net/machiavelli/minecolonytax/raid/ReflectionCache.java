 ...
 * This improves performance and provides centralized error handling for
...
public class ReflectionCache {
    ...
    // Keyed by "alliance_*", "harm_*", "entity_*", "additional_*" for fallback dispatch.
    ...
    // Multiple candidate class names handle different Recruits mod packaging versions.
    ...
    public static synchronized void initialize() {
        ...
        EntityRaidDebugLogger.logReflectionCall("initialize", "ReflectionCache", true,
                "Starting enhanced reflection cache initialization");
        ...
        if (recruitsModAvailable) {
            if (TaxConfig.isDebugLogging()) {
                LOGGER.info(
                        "ReflectionCache initialized in {}ms - Recruits mod detected and methods cached successfully.",
                        System.currentTimeMillis() - startTime);
            }
        } else {
            if (TaxConfig.isDebugLogging()) {
                LOGGER.info("ReflectionCache initialized in {}ms - Recruits mod not available, using fallback methods",
                        System.currentTimeMillis() - startTime);
            }
        }
    ...
    private static void initializeRecruitEventsClass() {
        ...
        if (recruitEventsClass == null) {
            EntityRaidDebugLogger.logReflectionCall("RecruitEvents", "all variants", false,
                    "No RecruitEvents class found");
            return;
        }
    ...
    private static void initializeRecruitEntityClass() {
        ...
        if (recruitEntityClass == null) {
            EntityRaidDebugLogger.logReflectionCall("RecruitEntity", "all variants", false,
                    "No recruit entity class found");
            return;
        }
    ...
    private static void cacheAllianceMethod(Class<?> clazz, String methodName, Class<?>[] paramTypes) {
        try {
            ...
            if (isAllyMethod == null) {
                isAllyMethod = method;
                EntityRaidDebugLogger.logReflectionCall(methodName, clazz.getSimpleName(), true,
                        "Primary alliance method cached");
            } else {
                dynamicMethodCache.put("alliance_" + methodName, method);
                EntityRaidDebugLogger.logReflectionCall(methodName, clazz.getSimpleName(), true,
                        "Fallback alliance method cached");
            }
    ...
    private static void cacheHarmMethod(Class<?> clazz, String methodName, Class<?>[] paramTypes) {
        try {
            ...
            if (canHarmTeamMethod == null) {
                ...
                EntityRaidDebugLogger.logReflectionCall(methodName, clazz.getSimpleName(), true,
                        "Primary harm method cached");
            } else {
                dynamicMethodCache.put("harm_" + methodName, method);
                EntityRaidDebugLogger.logReflectionCall(methodName, clazz.getSimpleName(), true,
                        "Fallback harm method cached");
            }
        } catch (NoSuchMethodException e) {
            EntityRaidDebugLogger.logReflectionCall(methodName, clazz.getSimpleName(), false, "Method not found");
        }
    ...
    public static Boolean isAlly(Team team1, Team team2) {
        ...
        for (Map.Entry<String, Method> entry : dynamicMethodCache.entrySet()) {
            if (entry.getKey().startsWith("alliance_")) {
                try {
                    ...
                    if (result instanceof Boolean) {
                        EntityRaidDebugLogger.logReflectionCall("isAlly", entry.getKey(), true,
                                "Fallback result: " + result);
                        return (Boolean) result;
                    }
                } catch (Exception e) {
                    EntityRaidDebugLogger.logError("ReflectionCache.isAlly",
                            "Error invoking fallback method " + entry.getKey(), e);
                }
            }
        }
        ...
        if (harmResult != null) {
            EntityRaidDebugLogger.logReflectionCall("isAlly", "inverted_harm", true,
                    "Inverted harm result: " + harmResult);
            return harmResult;
        }

        EntityRaidDebugLogger.logReflectionCall("isAlly", "all_methods", false,
                "All alliance detection methods failed");
        return null;
    }

    private static Boolean tryInvertedHarmCheck(Team team1, Team team2) {
        // Requires spawning dummy entities to test — not implemented; returns null.
        return null;
    }
    ...
    public static Boolean canHarmTeam(LivingEntity entity1, LivingEntity entity2) {
        ...
        if (!recruitsEventsAvailable) {
            EntityRaidDebugLogger.logReflectionCall("canHarmTeam", "ReflectionCache", false,
                    "RecruitEvents not available");
            return null; // Indicates method not available
        }
        ...
        if (canHarmTeamMethod != null) {
            try {
                ...
                EntityRaidDebugLogger.logError("ReflectionCache.canHarmTeam",
                        "Error invoking primary canHarmTeam method", e);
            }
        }
        ...
        for (Map.Entry<String, Method> entry : dynamicMethodCache.entrySet()) {
            if (entry.getKey().startsWith("harm_")) {
                try {
                    ...
                    if (result instanceof Boolean) {
                        EntityRaidDebugLogger.logReflectionCall("canHarmTeam", entry.getKey(), true,
                                "Fallback result: " + result);
                        return (Boolean) result;
                    }
                } catch (Exception e) {
        ...
        // isEnemy is semantically equivalent to canHarm when teams are enemies.
            ...
            try {
                ...
                if (team1 != null && team2 != null) {
                    ...
                    if (result instanceof Boolean) {
                        ...
                        EntityRaidDebugLogger.logReflectionCall("canHarmTeam", "isEnemy_inverted", true,
                                "Enemy result: " + isEnemy + ", can harm: " + isEnemy);
        ...
        EntityRaidDebugLogger.logReflectionCall("canHarmTeam", "all_methods", false,
                "All harm detection methods failed");
        return null;
    }
    ...
    public static UUID getOwnerUUID(Object entity) {
        ...
        if (!abstractRecruitEntityAvailable) {
            EntityRaidDebugLogger.logReflectionCall("getOwnerUUID", "ReflectionCache", false,
                    "Recruit entity classes not available");
            return null;
        }
        ...
        if (getOwnerUUIDMethod != null) {
            try {
                ...
                if (result instanceof UUID) {
                    EntityRaidDebugLogger.logReflectionCall("getOwnerUUID", "primary", true,
                            "UUID retrieved successfully");
                    return (UUID) result;
                }
            } catch (Exception e) {
                EntityRaidDebugLogger.logError("ReflectionCache.getOwnerUUID",
                        "Error invoking primary getOwnerUUID method", e);
            }
        }

        for (Map.Entry<String, Method> entry : dynamicMethodCache.entrySet()) {
            if (entry.getKey().startsWith("entity_") && entry.getKey().contains("Owner")
                    && entry.getKey().contains("UUID")) {
                try {
                    Object result = entry.getValue().invoke(entity);
                    if (result instanceof UUID) {
                        EntityRaidDebugLogger.logReflectionCall("getOwnerUUID", entry.getKey(), true,
                                "Fallback UUID retrieved");
                        return (UUID) result;
                    }
                } catch (Exception e) {
                    EntityRaidDebugLogger.logError("ReflectionCache.getOwnerUUID",
        ...
        if (getOwnerMethod != null) {
            try {
                ...
                if (result != null) {
                    ...
                    if (uuid != null) {
                        EntityRaidDebugLogger.logReflectionCall("getOwnerUUID", "getOwner_extracted", true,
                                "UUID extracted from owner object");
                        return uuid;
                    }
                }
            } catch (Exception e) {
        ...
        try {
            ...
            for (String methodName : methodNames) {
                try {
                    ...
                    if (result instanceof UUID) {
                        EntityRaidDebugLogger.logReflectionCall("getOwnerUUID", "dynamic_" + methodName, true,
                                "Dynamic UUID retrieved");
                        return (UUID) result;
                    } else if (result != null) {
                        UUID uuid = extractUUIDFromOwner(result);
                        if (uuid != null) {
                            EntityRaidDebugLogger.logReflectionCall("getOwnerUUID",
                                    "dynamic_" + methodName + "_extracted", true, "Dynamic UUID extracted");
                            return uuid;
                        }
                    }
        ...
        EntityRaidDebugLogger.logReflectionCall("getOwnerUUID", "all_methods", false,
                "All owner UUID retrieval methods failed");
        return null;
    }
    ...
    private static UUID extractUUIDFromOwner(Object owner) {
        ...
        try {
            ...
            EntityRaidDebugLogger.logError("ReflectionCache.extractUUIDFromOwner",
                    "Error extracting UUID from owner object", e);
        }
    ...
    public static Boolean isOwned(Object entity) {
        ...
        if (!abstractRecruitEntityAvailable) {
            EntityRaidDebugLogger.logReflectionCall("isOwned", "ReflectionCache", false,
                    "Recruit entity classes not available");
            return null;
        }
        ...
        for (Map.Entry<String, Method> entry : dynamicMethodCache.entrySet()) {
            if (entry.getKey().startsWith("entity_")
                    && (entry.getKey().contains("Owned") || entry.getKey().contains("Owner"))) {
                try {
                    Object result = entry.getValue().invoke(entity);
                    if (result instanceof Boolean) {
                        EntityRaidDebugLogger.logReflectionCall("isOwned", entry.getKey(), true,
                                "Fallback result: " + result);
                        return (Boolean) result;
                    }
                } catch (Exception e) {
                    EntityRaidDebugLogger.logError("ReflectionCache.isOwned",
                            "Error invoking fallback method " + entry.getKey(), e);
                }
            }
        }
        ...
        if (ownerUUID != null) {
            EntityRaidDebugLogger.logReflectionCall("isOwned", "inferred_from_uuid", true,
                    "Inferred ownership from UUID presence");
            return true;
        }
        ...
        try {
            ...
            for (String methodName : methodNames) {
                try {
                    ...
                    Object result = method.invoke(entity);
                    if (result instanceof Boolean) {
                        EntityRaidDebugLogger.logReflectionCall("isOwned", "dynamic_" + methodName, true,
        ...
        EntityRaidDebugLogger.logReflectionCall("isOwned", "all_methods", false,
                "All ownership detection methods failed");
        return null;
    }
    ...
    public static Long getCreationTime(Object entity) {
        ...
        if (getCreationTimeMethod != null) {
            try {
                ...
                EntityRaidDebugLogger.logError("ReflectionCache.getCreationTime",
                        "Error invoking cached getCreationTime method", e);
            }
        }
    ...
    public static Team getEntityTeam(Object entity) {
        ...
        if (getTeamMethod != null) {
            try {
                ...
                if (result instanceof Team) {
                    EntityRaidDebugLogger.logReflectionCall("getEntityTeam", "primary", true,
                            "Team retrieved successfully");
                    return (Team) result;
                }
            } catch (Exception e) {
                EntityRaidDebugLogger.logError("ReflectionCache.getEntityTeam", "Error invoking primary getTeam method",
                        e);
            }
        }
        ...
        if (teamMethod != null) {
            try {
                ...
                if (result instanceof Team) {
                    EntityRaidDebugLogger.logReflectionCall("getEntityTeam", "fallback", true,
                            "Team retrieved via fallback");
                    return (Team) result;
                }
            } catch (Exception e) {
                EntityRaidDebugLogger.logError("ReflectionCache.getEntityTeam",
                        "Error invoking fallback getTeam method", e);
            }
        }
        ...
        if (entity instanceof net.minecraft.world.entity.LivingEntity) {
            ...
            if (team != null) {
                EntityRaidDebugLogger.logReflectionCall("getEntityTeam", "minecraft_standard", true,
                        "Team retrieved via standard method");
                return team;
            }
        }

        EntityRaidDebugLogger.logReflectionCall("getEntityTeam", "all_methods", false,
                "All team retrieval methods failed");
        return null;
    }
    ...
    public static UUID getRecruitOwnerUUID(Object entity) {
        ...
        if (!abstractRecruitEntityAvailable || !isRecruitEntity(entity)) {
            EntityRaidDebugLogger.logReflectionCall("getRecruitOwnerUUID", "not_recruit", false,
                    "Entity is not a recruit entity");
            return null;
        }

        UUID ownerUUID = getOwnerUUID(entity);
        if (ownerUUID != null) {
            EntityRaidDebugLogger.logReflectionCall("getRecruitOwnerUUID", "success", true,
                    "Recruit owner UUID retrieved: " + ownerUUID.toString());
        } else {
            EntityRaidDebugLogger.logReflectionCall("getRecruitOwnerUUID", "failed", false,
                    "Failed to retrieve recruit owner UUID");
        }
