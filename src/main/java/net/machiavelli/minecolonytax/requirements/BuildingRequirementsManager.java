 ...
 * Supports both legacy format (building:level) and new format
...
public class BuildingRequirementsManager {
    ...
    private static RequirementResult checkBuildingRequirements(IColony colony, String requirements, String actionType) {
        if (net.machiavelli.minecolonytax.TaxConfig.isDebugLogging()) {
            LOGGER.info("Checking building requirements for {}: '{}' (Colony ID: {})", actionType, requirements,
                    colony.getID());
        }
        ...
        try {
            ...
            for (BuildingRequirement requirement : parsedRequirements) {
                ...
                int availableCount = countBuildingsOfTypeAndLevel(colony, requirement.buildingType,
            ...
            if (missingRequirements.isEmpty()) {
                ...
                String message = "Missing building requirements for " + actionType + ": "
            ...
            return new RequirementResult(false,
                    "Error checking building requirements - denying " + actionType + " for safety.");
        }
    }
    ...
    private static int countBuildingsOfTypeAndLevel(IColony colony, String buildingType, int minLevel) {
        ...
        try {
            LOGGER.debug("Colony {}: checking for {} level {}+", colony.getID(), buildingType, minLevel);
    ...
    private static boolean matchesBuildingType(String actualType, String requiredType) {
        ...
        return switch (requiredType) {
            ...
            case "house", "residential" ->
                actualType.contains("house") || actualType.contains("residence") || actualType.contains("citizen");
