...
public class BesiegeDamageShieldHandler {
    ...
    private static boolean areColonyMates(ServerPlayer source, BesiegeManager.BesiegeRaidData raid) {
        ...
        // Short-TTL cache per source — LivingIncomingDamageEvent is extremely high-frequency
        // and the colony scan below is O(allColonies). The TTL collapses a combat hit-burst
    ...
    private static boolean isDefenderSideTarget(LivingEntity target, BesiegeManager.BesiegeRaidData raid) {
        ...
        if (target instanceof ServerPlayer player) {
            try {
                // Dimension-safe: resolving the wrong colony here decides who may damage whom.
