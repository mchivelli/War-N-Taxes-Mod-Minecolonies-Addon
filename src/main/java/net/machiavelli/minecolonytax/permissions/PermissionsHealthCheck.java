...
public class PermissionsHealthCheck {
     ...
     * attackers can fight, and the attacker/raider colony so the defenders can strike back). But
     * ACTIVE_WARS is keyed by DEFENDER colony id and getActiveRaidForColony() matches only the TARGET
     * colony, so a containsKey/lookup alone sees just one side. Without the attacker-side terms below,
     * the cleanup pass would strip the war nodes off the attacking colony (e.g. on the startup pass
     * after a war survives a restart, or on a manual /wnt permcheck), leaving its defenders unable to
     * fight back there. This mirrors the attacker-side handling isLegitimatelyHostile() already does
     * for ranks.
     */
