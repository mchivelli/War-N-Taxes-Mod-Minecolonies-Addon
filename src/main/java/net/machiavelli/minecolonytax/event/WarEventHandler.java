...
public class WarEventHandler {
    ...
    // Track players who have disconnected during active wars/raids for reconnection
    // handling
    ...
    public static void onPlayerDeath(LivingDeathEvent event) {
        ...
        // === RANSOM SYSTEM: Check if a DEFENDER died during an active raid ===
        // If a colony member dies while their colony is being raided, trigger ransom
        // system
        if (TaxConfig.isRansomSystemEnabled()) {
            for (ActiveRaidData activeRaid : RaidManager.getActiveRaids().values()) {
                if (activeRaid.isActive() && activeRaid.getColony() != null) {
                    // Check if dead player is a member of the colony being raided (defender)
                    if (activeRaid.getColony().getPermissions().getPlayers().containsKey(player.getUUID())) {
                        // Dead player is a defender - trigger ransom offer from the raider
                        UUID raiderUUID = activeRaid.getRaider();
                        int colonyId = activeRaid.getColony().getID();

                        // Create ransom offer (this checks cooldowns, online status, etc.)
                        boolean offerCreated = net.machiavelli.minecolonytax.economy.RansomManager
                                .createRansomOffer(raiderUUID, player.getUUID(), colonyId);

                        if (offerCreated) {
                            System.out
                                    .println("[DEBUG] Ransom offer created for defender " + player.getName().getString()
                                            + " killed during raid on colony " + colonyId);
                        }
                        break; // Only one ransom offer per death
                    }
                }
            }
        }
        ...
        System.out.println(
                "[DEBUG] WarEventHandler.onPlayerDeath triggered confirmed for " + player.getName().getString());

        // Check if war is still in join phase - don't deduct lives during join phase
        if (war.isJoinPhaseActive()) {
            System.out.println(
                    "[DEBUG] War is still in join phase, not deducting lives for " + player.getName().getString());
            return;
        }
        ...
        // If player is on their last life and keepInventoryOnLastLife is enabled,
        // prepare for inventory preservation
        if (currentLives == 1 && TaxConfig.KEEP_INVENTORY_ON_LAST_LIFE.get()) {
            System.out.println("[DEBUG] Player " + player.getName().getString()
                    + " is on last life with keepInventoryOnLastLife enabled - preserving inventory on respawn");
            ...
            System.out
                    .println("[DEBUG] Inventory saved for last life preservation for " + player.getName().getString());
        }
        ...
        System.out.println(
                "[DEBUG] Player " + player.getName().getString() + " died during war. Lives map before: " + lives);
        ...
        if (lives.isEmpty()) {
            System.out.println("[DEBUG] Player " + player.getName().getString()
                    + " is not participating in any war, skipping lives processing");
            return;
        }
        ...
        if (!lives.containsKey(playerUUID)) {
            System.out.println("[DEBUG] Player " + player.getName().getString()
                    + " was not in lives map, initializing with " + defaultLives + " lives");
        ...
        int remaining = lives.compute(playerUUID, (k, v) -> {
            if (v == null || v <= 0) {
                ...
                System.out.println("[DEBUG] Lives value was null or invalid (" + v + "), using default " + defaultLives
                        + " minus 1");
        ...
        // The actual life processing will happen on respawn to ensure proper death
        // mechanics
        // DO NOT cancel the event - let natural death processing occur (corpses, death
        // messages, etc.)
    }
    ...
    public static void onPlayerRespawn(PlayerEvent.PlayerRespawnEvent event) {
        ...
        // Check if this is an empty map (player not in war) - skip processing if so
        if (lives.isEmpty()) {
            System.out.println("[DEBUG] RESPAWN - Player " + player.getName().getString()
                    + " is not participating in any war, skipping respawn processing");
            return;
        }

        int currentLives = lives.getOrDefault(playerUUID, 0);

        System.out.println("[DEBUG] RESPAWN - Player " + player.getName().getString() + " respawned with "
                + currentLives + " lives remaining");

        // Check if this player died on their last life and we need to restore their
        // inventory
        if (war.getLastLifeInventoryPreservation().contains(playerUUID) && savedInventories.containsKey(playerUUID)) {
            ...
            player.sendSystemMessage(
                    Component.literal("You have died on your last life! You are now a spectator until the war ends.")
                            .withStyle(style -> style.withColor(ChatFormatting.GOLD).withBold(true)));

            // Confirm to console
            System.out.println(
                    "[INFO] Restored inventory for player " + player.getName().getString() + " after last life death");
            ...
            if (TaxConfig.ENABLE_COLONY_TRANSFER.get() && TaxConfig.KEEP_INVENTORY_ON_LAST_LIFE.get()) {
                player.sendSystemMessage(Component
                        .literal("If you die, you will keep your inventory and can continue fighting as a spectator.")
    ...
    public static void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        // Always check if player belongs to a colony that's being raided (defender
        // logic)
        // This catches cases where a defender wasn't properly tracked or was offline
        // when raid started
    ...
    public static void onCitizenDeath(LivingDeathEvent event) {
        ...
        if (war.getGuardIDs().remove(data.getId())) {
            ...
            // We assume that if the guard's colony equals the attacked colony, then it's
            // defender;
            ...
            if (citizenColony.equals(war.getColony())) {
                ...
                // Default announcement if we cannot determine affiliation - only to war
                // participants
            ...
            // Play guard death sound at the location of the fallen guard using Minecraft's
            // villager hurt sound
