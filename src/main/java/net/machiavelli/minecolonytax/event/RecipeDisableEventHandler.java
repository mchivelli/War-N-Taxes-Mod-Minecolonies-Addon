...
/**
 * Event handler for disabling Minecolonies building hut recipes when
 * configured.
 * This ensures that buildings with taxes/maintenance costs must be obtained
 * through shops.
 */
...
public class RecipeDisableEventHandler {
    ...
    // Set of building hut blocks that have taxes or maintenance costs and should
    // have recipes disabled
    ...
    static {
        ...
        // These are all the buildings that have taxes or maintenance costs defined in
        // TaxConfig
    ...
    public static void onAddReloadListener(AddReloadListenerEvent event) {
        ...
        if (TaxConfig.isDisableHutRecipesEnabled()) {
            if (TaxConfig.isNormalLogging()) {
                LOGGER.info(
                        "RecipeDisabler: Intercepted recipes sync. Preparing to disable {} hut recipes on clients.",
                        DISABLED_HUT_RECIPES.size());
