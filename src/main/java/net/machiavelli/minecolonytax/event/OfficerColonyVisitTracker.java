 ...
 *
 * DEFAULT BEHAVIOR: Timer resets when an owner or officer physically ENTERS
 * their colony (chunk-based detection).
 * This ensures officers must actually visit their colonies to prevent
 * abandonment.
 *
 * OPTIONAL: Login tracking can be enabled via config (ResetTimerOnOfficerLogin
 * = true).
 * When enabled, timer also resets when officers log into the server (for ALL
 * colonies they manage).
...
public class OfficerColonyVisitTracker {
    ...
    // Track last login time for each colony by officers/owners (colonyId ->
    // timestamp in millis)
    private static final Map<Integer, Long> lastOfficerVisit = new ConcurrentHashMap<>();

    // Track each player's last known colony ID for entry detection (playerUUID ->
    // colonyId, -1 if not in colony)
     ...
     * For colonies without WnT data, initializes their timer based on MineColonies'
     * lastContactInHours.
