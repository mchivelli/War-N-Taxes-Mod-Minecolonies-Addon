...
public final class CorpseRetrievalHandler {
    ...
    public static void onEntityInteract(PlayerInteractEvent.EntityInteract event) {
        allowOwnCorpse(event, event.getTarget());
    }
    ...
    private static void allowOwnCorpse(PlayerInteractEvent event, Entity target) {
        try {
            if (!event.isCanceled()) return;                       // nothing to undo
            ...
            // The cancellation result is what the interaction reports back once cancelled; reset it
            // so the corpse's own interact handler decides the outcome rather than the stale deny.
