...
The Besiege system lets a player raid another player's non-primary colony and, on victory, extract tribute from it — without triggering a full server-wide war.

---

## When to Use It

Use **besiege** when you want to contest a player's secondary colony for tax income. Besieging is a single-player action (though allies can help) with lower political costs than war.

**First (primary) colonies cannot be besieged.** To contest a player's primary colony you must declare full war.
...
- The target must not be the owner's primary colony.
- The target must have at least 5 citizens (configurable).
- The target must not already have an active besiege raid.
...
- The target colony is **force-vassalized** — it pays a percentage of its tax income (default 30%) to the besieger each tax cycle.
- The **former owner is locked out** of colony management. They cannot open chests, access building GUIs, or place/break blocks in the colony.
- The vassalage lasts for a configurable duration (default 72 hours), after which it expires automatically.
...
| **Besiege** | Single non-primary colony | Tax vassalage + management lockout |
| **War** | Multiple colonies, server-wide | Occupation, vassalage, or colony transfer |
